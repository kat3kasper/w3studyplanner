...
accept_loop(Socket) :-
        ...
        ensure_loaded('/opt/apache/htdocs/studyplanner/lib/degreeChecker_final.pl'),read_exdr(ConSocket, Goal), write(Goal), nl, timeout(Goal, 60, Goal = Goal), write_exdr(ConSocket, Goal), Goal = okDegree([],[],[]),
        close(ConSocket),
        accept_loop(Socket).
